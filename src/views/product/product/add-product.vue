    ...
    <div>
                    ...
                    <Row type="flex"  align="middle">
                        <Col span="8">               	
	                    	<FormItem prop="name" label="商品名称" >
	                    	    <Input v-model="formValidate.name" placeholder="请填写商品名称..." class="form-element-base"></Input>
	                        </FormItem>
                        </Col>
                        <Col offset="5" span="8">  
	                    	<FormItem prop="supplierCode" label="商品编码" >
	                    	    <Input v-model="formValidate.supplierCode" placeholder="请输入商品编码..." ></Input>
	                        </FormItem>
                        </Col>
                    </Row>
                    ...
                    <Row class="margin-top-8" type="flex"  align="middle">
                    	<Col span="8">  
		                    <FormItem prop="brandId" label="品牌" >
		                	    <Select v-model="formValidate.brandId">
                        ...
                        <Col offset="5" span="8">  
	                        <FormItem prop="productTypeId" label="商品类别"  >
	                            <Select v-model="formValidate.productTypeId" >
                    ...
                    <Row class="margin-top-8" type="flex"  align="middle">
                    	<Col span="8">  
	                    	<FormItem prop="isVirtua" label="是否虚拟商品" >
	                    	     <i-switch v-model="formValidate.isVirtua" ></i-switch>
	                        </FormItem>
                        </Col>
                    </Row>
                    </Form>
        ...
        <Row >
            <div v-for="(item,index) in skuFormValidate.skuList" >
	            <Col span="12" class="margin-top-8 padding-left-10">
	                <Card>
	                    <p slot="title">
	                        <Icon type="ios-list"></Icon>
	                        sku详细信息-{{index + 1}}
	                        <a href="#" slot="extra" @click.prevent="delFormSkuList(index)" class="ivu-card-extra">
	                    ...
	                    <Row type="flex"  align="middle">
	                        <Col span="11">               	
		                    	<FormItem :prop="'skuList.' + index + '.name'" label="sku名称" :rules="{required: true, message: 'sku名称不能为空', trigger: 'blur'}">
		                    	    <Input v-model="item.name" placeholder="请填写sku名称..." class="form-element-base"></Input>
		                        </FormItem>
	                        </Col>
	                        <Col offset="2" span="11">  
		                    	<FormItem :prop="'skuList.' + index + '.extCode1'" label="sku编码"  >
	                    ...
	                    <Row  type="flex"  align="middle">
	                    	<Col span="11">  
		                    	<FormItem :prop="'skuList.' + index + '.barCode'" label="条形码" >
		                    	    <Input v-model="item.barCode" placeholder="请输入条形码..." ></Input>
		                        </FormItem>
	                        </Col>
                            <Col offset="2" span="11">  
                                <FormItem :prop="'skuList.' + index + '.color'" label="sku颜色" >
	                    ...
	                    <Row  type="flex"  align="middle">
	                    	<Col span="11">  
		                    	<FormItem :prop="'skuList.' + index + '.price'" label="sku价格"  :rules="{ required:true,validator: validatePrice, trigger: 'blur' }">
		                    	    <Input v-model="item.price" placeholder="请输入sku价格..." ></Input>
		                        </FormItem>
	                        </Col>
	                        <Col offset="2" span="11">  
		                    	<FormItem :prop="'skuList.' + index + '.skuSize'" label="sku规格" >
		                    	    <Input v-model="item.skuSize" placeholder="请输入规格..." ></Input>
		                        </FormItem>
	                        </Col>
	                    </Row>
	                    <Row class="margin-top-8" type="flex"  align="middle" v-if="formValidate.isVirtua == true">
                            <Col span="11">
                                <FormItem :prop="'skuList.' + index + '.isRepeat'" label="重复购买" >
                                ...
                                <FormItem label="封面图片" :prop="'skuList.' + index + '.url'"  :rules="{validator: validateUrl }" class="margin-top-10">
                                    <Upload name="fileField" 
                                        ...
                                        <Button type="ghost" icon="ios-cloud-upload-outline" long @click="changeCurrentIndex($event)"
                                         :name="'upload_url_but' + index">上传封面图片</Button>
                                    </Upload>
                                </FormItem>
                            </Col>
                            <Col>
            	...
            	<Col span="12" class="margin-top-8 padding-left-10">
	                <Card>
	                   <div  style="height:370px" class="flex-middle-but">
                ...
                if (valid) {
                	if(skuFormValidFlag == 1){
		                ...
		                params:{
		                    ...
		                    skuListStr:JSON.stringify(this.skuFormValidate.skuList),
		                },
		                responseType:'json'
		                }).then(res=> {
			            	//console.log(res);
